...
func LatestVersions(releases []*semver.Version, minVersion *semver.Version) []*semver.Version {
	...
	for i :=  range releases{
		if minVersion < releases[i]{
			versionSlice[j] = releases[i]
		}
	}
	
	ver start := 0
	
	for i := range versionSlice{
		for j:= start ; j < range versionSlice ; j++{
			if versionSlice[start] < cersionSlice[j]{
				ver temp := versoinSlice[start]
				versoinSlice[start] := cersionSlice[j]
				cersionSlice[j] := temp
